...
export function HeroSection() {
  return (
    <section className="pt-24 md:pt-32 lg:pt-40 pb-16 md:pb-24 lg:pb-32 text-center relative z-10" id="hero">
      <div className="max-w-[1400px] mx-auto px-4 md:px-8">
        <div className="max-w-5xl mx-auto opacity-0 animate-[fadeInUp_0.8s_ease-out_0.15s_forwards]">
          <div className="inline-flex items-center space-x-2 md:space-x-3 bg-gradient-to-r from-primary/40 to-[oklch(0.60_0.22_250/0.4)] border border-primary/30 rounded-full pl-4 md:pl-6 pr-5 md:pr-8 py-2 md:py-3 mb-6 md:mb-10 shadow-inner group cursor-default hover:shadow-[0_0_30px_oklch(0.75_0.16_195/0.4)] transition-all duration-300">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-extrabold mb-6 md:mb-8 leading-[1.1] md:leading-[1.05]">
            Unleash the
            <br />
          ...
          <p className="text-base md:text-xl lg:text-2xl text-gray-300 mb-8 md:mb-14 max-w-3xl mx-auto leading-relaxed px-4">
...
function StatsSection() {
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8 mt-12 md:mt-24 pt-8 md:pt-12 border-t border-gray-800/50">
...
function StatBox({ value, prefix = '', suffix = '', text, label, delay }: StatBoxProps) {
  ...
  return (
    <div
      ref={ref}
      className={`bg-white/2 border border-white/7 rounded-xl md:rounded-2xl p-4 md:p-6 text-center backdrop-blur-[15px] shadow-[0_2px_20px_rgba(0,0,0,0.2)] transition-opacity duration-800 ${
        isVisible ? 'opacity-100' : 'opacity-0'
      }`}
    >
      <div className="text-gradient font-extrabold text-3xl md:text-4xl lg:text-5xl mb-2 md:mb-3">
        {text || `${prefix}${value === 99.999 ? currentValue.toFixed(3) : value === 1 ? Math.floor(currentValue) : Math.floor(currentValue)}${suffix}`}
      </div>
      <div className="text-muted-foreground font-semibold text-xs md:text-sm lg:text-base tracking-wide">
        {label}
      </div>
    </div>
  )

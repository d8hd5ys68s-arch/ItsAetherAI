...
export function TechnologySection() {
  return (
    <section id="technology" className="py-16 md:py-28 relative z-10">
...
function TechCard({ icon: Icon, title, description, gradient, delay }: TechCardProps) {
  ...
  return (
    <div
      ref={ref}
      className={`glass-card p-6 md:p-10 text-center transition-all duration-800 ${
    ...
    >
      <div
        className={`w-20 h-20 md:w-24 md:h-24 bg-gradient-to-r ${gradient} rounded-2xl md:rounded-3xl flex items-center justify-center mx-auto mb-6 md:mb-8 shadow-xl transition-all duration-400 ${
      ...
      >
        <Icon size={36} weight="fill" className="text-white md:w-12 md:h-12" />
      </div>
      <h3 className="text-xl md:text-2xl font-semibold mb-3 md:mb-4 text-white">{title}</h3>
